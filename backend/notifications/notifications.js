...
class NotificationsManager {
    ...
    async notifyUsersByEmail() {
        ...
        try {
            let accountIDs = await notificationsDb.aggregate([
                { $match: { status: 'unread' }},
                { $group: { _id: '$account' }},
            ]);
            ...
            for (const accountID of accountIDs) {
                ...
                if(emailAddresses.length) {
                    await mailer.sendMailHTML(
                        ...
                        <p style="font-size:16px">This email was sent to you since you have pending unread notifications.
                            <br>To view the notifications, please check the <a href="${configs.get('UIServerURL')}/notifications">Notifications</a> page in your flexiMange account.</br>
                        </p>
                        <p style="font-size:14px;color:gray">Note: Unread notification email alerts are sent to Account owners (not Users in Organization level). You can disable these emails in the <a href="${configs.get('UIServerURL')}/accounts/update">Account profile</a> page in your flexiManage account. Alerts on new flexiEdge software versions or billing information are always sent, regardless of the notifications settings. More about notifications <a href="https://docs.flexiwan.com/troubleshoot/notifications.html">here</a>.</p>
