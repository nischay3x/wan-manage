        ...
        .then((msg) => {
            if (msg != null) {
                ...
                logger.warn("Failed to get device status", {
                    params: {deviceID: deviceID, message: msg}, periodic: {task: this.taskInfo}}
                );
            }
        }, (err) => {
            logger.warn("Failed to get device status", {
                params: {deviceID: deviceID, err: err.message}, periodic: {task: this.taskInfo}}
            );
            return err;
        })
        .catch((err) => {
