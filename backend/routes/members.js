...
.get(cors.corsWithOptions, verifyPermission('members', 'get'), async (req,res,next) => {
    ...
    else if (req.user.perms.organizations & permissionMasks.get) userPromise = membership.find({account:req.user.defaultAccount._id, '$or': [{to:'organization', organization: req.user.defaultOrg._id},
...
.get(cors.corsWithOptions, verifyPermission('members', 'get'), async (req,res,next) => {
    ...
    if (req.params.optionsType === 'account') return res.status(200).json([{
        'id':req.user.defaultAccount._id, 'value':req.user.defaultAccount.name}]);
...
.get(cors.corsWithOptions, verifyPermission('members', 'get'), async (req,res,next) => {
    ...
    // Account owners or members should be able to see all account users + groups + all organizations users
    // Organization members should be able to see all organization users
    if (req.user.perms.accounts & permissionMasks.get) userPromise = membership.find({
        _id:req.params.memberId, 
        account:req.user.defaultAccount._id});
    else if (req.user.perms.organizations & permissionMasks.get) userPromise = membership.find({
        _id:req.params.memberId, 
        account:req.user.defaultAccount._id, 
