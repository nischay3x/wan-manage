...
tunnelsRouter
    ...
    .get(cors.corsWithOptions, verifyPermission('tunnels', 'get'), (req,res,next) => {
        ...
        resp.forEach((d) => {
            ...
            d.deviceA.interfaces.filter((interface)=>{return interface._id == ""+d.interfaceA})[0],
            {strict: false});
            d.set('interfaceBDetails', 
            d.deviceB.interfaces.filter((interface)=>{return interface._id == ""+d.interfaceB})[0],
            {strict: false});

            try {
                const tunnelId = d.num;
                // Add tunnel status
                d.set("tunnelStatusA", deviceStatus.getTunnelStatus(d.deviceA.machineId, tunnelId) || null,
                    { strict: false });

                // Add tunnel status
                d.set("tunnelStatusB", deviceStatus.getTunnelStatus(d.deviceB.machineId, tunnelId) || null,
                    { strict: false });
            } catch (error) {
                // console.error(error);
            }
