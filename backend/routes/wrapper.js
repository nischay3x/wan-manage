...
exports.assignRoutes = function (router, permissionResource, subpath, model, formatErr, checkUpdReq = defaultCheckReq, checkUpdResp = defaultCheckResp) {
  ...
  const isField = !((idIndex == -1 || fieldIndex == -1 || fieldIndex <= idIndex));
        ...
        checkUpdReq('DELETE', req)
          .then((checkreq) => {
            ...
            if (isField) {
              ...
              var p = model.findOneAndDelete({ _id: mongoose.Types.ObjectId(req.params[idName]), org: req.user.defaultOrg._id })
